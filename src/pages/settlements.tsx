...
function statusVariant(status?: string): 'success' | 'warning' | 'danger' | 'secondary' | 'gold' {
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button variant="gold" disabled={settling} onClick={() => void submitInstantSettle()}>
                Confirm settle
              </Button>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
                  <tr>
                    ...
                    <th className="px-3 py-2">Fees+Tax</th>
                ...
                <tbody>
                  {rows.map((row) => (
                    ...
                    >
                      <td className="px-3 py-2 whitespace-nowrap">{formatDateTime(row.gatewayCreatedAt)}</td>
                      <td className="px-3 py-2">{row.kind}</td>
                      <td className="px-3 py-2">
                        <Badge variant={statusVariant(row.status)}>{row.status}</Badge>
                      </td>
                      <td className="px-3 py-2 font-medium">{formatCurrency(row.amount)}</td>
                      <td className="px-3 py-2">{formatCurrency((row.fees ?? 0) + (row.tax ?? 0))}</td>
                      <td className="px-3 py-2 font-mono text-xs">{row.utr || '-'}</td>
                      <td className="px-3 py-2 font-mono text-xs">{row.gatewaySettlementId}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
