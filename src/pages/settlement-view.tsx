...
function CopyableValue({
  ...
  return (
    <p className="flex flex-wrap items-start gap-2 break-all">
      ...
      <button
        type="button"
        className="inline-flex max-w-full items-center gap-1.5 rounded-md px-1.5 py-0.5 font-mono text-xs text-slate-700 hover:bg-muted"
      ...
      <PageToolbar
        ...
        description={
          <span className="inline-flex flex-wrap items-center gap-2">
            <Badge variant="secondary">{settlement.kind}</Badge>
                ...
                <tbody>
                  {settlement.items.map((item) => (
                    <tr key={item.id} className="border-t border-border/70">
                      ...
                      <td className="px-3 py-2">
                        {item.settled ? (
                          <span className="inline-flex items-center gap-1.5 text-emerald-700" title="Settled">
                            <CheckCircle2 className="h-4 w-4" />
